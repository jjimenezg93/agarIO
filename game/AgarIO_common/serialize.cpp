...
namespace aioc {
	int SerializeCommand(CBuffer &outBuffer, void * data,
		...
		switch (command) {
			...
			{
				//inBuffer = reinterpret_cast<CBuffer *&>(data);
				inBuffer->GotoStart();
				inBuffer->Read(&playerID, sizeof(playerID));
			...
			case C_PLAYERS_SNAPSHOT:
			{
				...
				//std::vector<aioc::CEntity *> *entities =
				//	reinterpret_cast<std::vector<aioc::CEntity *> *&>(data);
				...
				if (numPlayers > 0) {
					...
					//std::vector<aioc::CEntity *>::iterator eItr = entities->begin();
